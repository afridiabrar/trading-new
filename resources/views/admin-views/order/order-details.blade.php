        ...
        <div class="page-header d-print-none p-3" style="background: white">
            <div class="row align-items-center">
                <div class="col-sm mb-2 mb-sm-0">
                    ...
                    <div class="hs-unfold float-right pr-2">
                        <div class="dropdown">
                            <select name="tracking_status" onchange="order_tracking_status(this.value);"
                                    class="payment_status form-control"
                                    data-id="{{$order['id']}}">
                                <option value="" hidden>Select Order Tracking</option>

                                <option value="delivered" {{$order->tracking_status == 'delivered'?'selected':''}} >
                                    {{trans('messages.Delivered')}}
                                </option>
                                <option value="order_on_way" {{$order->tracking_status == 'order_on_way'?'selected':''}} >
                                    {{trans('messages.Order_On_Way')}}
                                </option>
                                <option value="order_picked_up_from_location" {{$order->tracking_status == 'order_picked_up_from_location'?'selected':''}} >
                                    {{trans('messages.Order_Picked_Up_From_Location')}}
                                </option>
                                <option value="waiting_for_driver_assignment" {{$order->tracking_status == 'waiting_for_driver_assignment'?'selected':''}} >
                                    {{trans('messages.Waiting_For_Driver_Assignment')}}
                                </option>
...
@push('script_2')
    <script>
        ...
        function order_tracking_status(status) {
            var value = status;
            // Swal.fire({
            //     title: 'Are you sure Change this?',
            //     text: "You won't be able to revert this!",
            ...
            // }).then((result) => {
                if (value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.orders.tracking.status')}}",
                        method: 'POST',
                        data: {
                            "id": '{{$order['id']}}',
