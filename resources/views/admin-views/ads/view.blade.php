    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{trans('messages.ads')}}</li>
            </ol>
        </nav>
        <!-- Page Heading -->
        <div class="row">
            <div class="col-md-12" id="banner-btn">
                <button id="main-banner-add" class="btn btn-primary"><i
                        class="tio-add-circle"></i> {{ trans('messages.ads_add')}}</button>
                            ...
                            <div class="form-group">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <label for="name">{{ trans('messages.ads_url_name')}}</label>
                                        <input type="text" max="255" name="button_name" class="form-control" id="button_name" required>

                                        <label for="name">{{ trans('messages.ads_url')}}</label>
                                        ...
                                        <label for="name">{{ trans('messages.ads_image')}}</label><span
...
{{--        </div>--}}
        ...
        <div class="row" style="margin-top: 20px" id="banner-table">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h5>{{ trans('messages.ads_table')}}</h5>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table id="columnSearchDatatable"
                                ...
                                <tr>
                                    ...
                                    <th>{{trans('messages.ads_title')}}</th>
                                    <th>{{trans('messages.ads_sub_title')}}</th>
                                    <th>{{trans('messages.ads_url_name')}}</th>
                                    <th>{{trans('messages.ads_url')}}</th>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    <a class="dropdown-item  edit" style="cursor: pointer;"
                                                       id="{{ $ad->id }}"> {{ trans('messages.Edit')}}</a>
                                                    <a class="dropdown-item delete" style="cursor: pointer;"
                                                       id="{{ $ad->id }}"> {{ trans('messages.Delete')}}</a>
                                                </div>
                                            </div>
...
@push('script')
    ...
    <script>
        ...
        $(document).on('change', '.status', function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    console.log(data);
                    if (data == 1) {
                        toastr.success('Ad published successfully');
                    } else {
                        toastr.success('Ad unpublished successfully');
                    }
                }
            });
        });
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        data: {id: id},
                        success: function () {
                            toastr.success('Ads deleted successfully');
                            location.reload();
                        }
                    });
                }
        ...
        $(document).on('click', '.edit', function () {
            ...
            $.ajax({
                url: "{{route('admin.ads.update')}}?id=update",
                method: 'GET',
                data: {id: id},
                success: function (data) {
                    $('.banner_form').attr('action', "{{route('admin.ads.update')}}/"+id);
                    // console.log(data);
